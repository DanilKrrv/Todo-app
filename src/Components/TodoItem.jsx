...
const TodoItem = ({ colorMode, todo, handleCompletedTodo, handleDeleteTodo, index }) => {
            ...
            <Flex alignItems={'center'} minW={'100%'}>
              ...
              <Flex ml={'1em'} justifyContent={'space-between'} w={'100%'}>
                <Text
                  ...
                  color={isCompleted ? 'grey' : colorMode == 'light' ? 'black' : 'white'}
                >
                  {todo.title}
                </Text>
                {isVisible && (<Box cursor={'pointer'} onClick={() => handleDeleteTodo(todo.id)}>
                  <CloseIcon />
                </Box>)}
              </Flex>
            </Flex>
